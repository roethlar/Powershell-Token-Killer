    ...
    internal static async Task EscalateAsync(Process process, bool stopped)
    {
        ...
        if (state.Job is not null)
        {
            if (stopped)
            {
                ...
                {
                    // A sweep-only request must never turn uncertainty about
                    // the root into a fresh root termination request. The
                    // terminal release path still closes the job handle.
                    return;
                }
            }
