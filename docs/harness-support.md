...
| codex | `codex mcp add ptk -- <exe>` → `~/.codex/config.toml`. LIVE (tools listed) | `mcp__ptk.ptk_state` (dot separator) | none shipped (hooks trust-gated; unresolved) | `~/.codex/AGENTS.md` (self-report, high confidence; verify when the nudge lands) |
